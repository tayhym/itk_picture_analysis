//
//  registration_mutual_information.cpp
//  pictureAnalysis
//
...
#include "registration_mutual_information.h"

MutualInformationRegistration::MutualInformationRegistration(char *image_name_one, char *image_name_two) {
  fixed_image = image_name_one;
  moving_image = image_name_two;
}
typedef itk::Image<unsigned char,2> OutputImageType;

OutputImageType::Pointer MutualInformationRegistration::start_registration() {
  std::cout << "starting mutual information registration" << std::endl;
...
//  typedef itk::AffineTransform< double, Dimension >         TransformType;
  ...
  //TODO: CHANGE THE NUMBER OF BINS FOR OPTIMAL PERFORMANCE
  numberOfBins = 1024;
  // Software Guide : BeginCodeSnippet
  ...
  // Software Guide : BeginCodeSnippet
  double samplingPercentage = 0.50;  // CHANGED SAMPLING PERCENTAGE 0.2 -> 0.5
  // Software Guide : EndCodeSnippet
  ...
  //TODO: alter the value of the defaultPixelValue parameter
  ...
  writer->SetFileName( "mutual_info_after_registration.jpg" );
  
  caster->SetInput( resample->GetOutput() );
  writer->SetInput( caster->GetOutput()   );
  writer->Update();
  OutputImageType::Pointer outputPointer = caster->GetOutput(); // returns this value
  //  Software Guide : BeginLatex
  //
  // \begin{figure}
  // \center
  ...
  resample->SetTransform( identityTransform );
    ...
    writer->SetFileName( "mutual_information_identity_transform.jpg" );
    writer->Update();
